...
// infinitive pipelines
void
process_commands(int num_params, char **params, int background)
{
	int pos, lon_palabra, status, this_pos, following_pos,
	    fds[num_params][2];
	...
	for (pos = 0; pos < num_params; pos++) {
		...
		lon_palabra = strlen(params[pos]);

		commands[pos].args = malloc((lon_palabra + 5) * sizeof(char *));

		token_arg(params[pos], commands[pos].args);
		...
		commands[pos].new_path1 =
		    malloc(sizeof(char) * (total_length1 + 1));
		commands[pos].new_path2 =
		    malloc(sizeof(char) * (total_length2 + 1));
		strcat(strcpy(commands[pos].new_path1, path1),
		       commands[pos].args[0]);
		strcat(strcpy(commands[pos].new_path2, path2),
		       commands[pos].args[0]);
		...
		if (pos == 0) {
			...
			starting_fork(fds[0][READ_END], fds[0][WRITE_END],
				      commands[pos]);
		} else if (pos == num_params - 1) {
			end_fork(fds[num_params - 2][READ_END],
				 fds[num_params - 2][WRITE_END], commands[pos]);
			...
			middle_fork(fds[this_pos][READ_END],
				    fds[following_pos][READ_END],
				    fds[following_pos][WRITE_END],
				    commands[pos]);
			this_pos++;
			following_pos++;
		}
