        ...
        if (ent->d_name[0] != '.') {
			if (ent->d_type == DT_REG && typefile(ent->d_name) == 1) {
				...
				while ((nr = read(origin_file, buffer, BUF_SIZE)) != 0) {
					if (nr < 0) {
						close_pipes(NO_PIPE2CLOSE, fd_write, fd2_read,fd2_write);
...
process_dir(char *dir)
{
	...
	{
		close_pipes(fd[READ_END], fd[WRITE_END], fd2[READ_END],fd2[WRITE_END]);
    ...
    switch (fork()) {
		...
		close_pipes(NO_PIPE2CLOSE, fd[WRITE_END],NO_PIPE2CLOSE,fd2[WRITE_END]);
