...
{
	int pos, lon_palabra, status, fd1[2], fd2[2];
	...
	for (pos = 0; pos < num_params; pos++) {
		...
		if (pos == 0) {
			...
			starting_fork(fd1[READ_END], fd1[WRITE_END],
				      commands[pos]);
		} else if (pos == num_params - 1) {
			end_fork(fd2[READ_END], fd2[WRITE_END], commands[pos]);
			close(fd2[READ_END]);
		} else {
			if (pipe(fd2) < 0)
				err(EXIT_FAILURE, "cannot make a pipe");

			middle_fork(fd1[READ_END], fd2[READ_END],
				    fd2[WRITE_END], commands[pos]);
		}
	}
